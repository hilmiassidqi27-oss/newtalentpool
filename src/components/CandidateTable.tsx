...
export default function CandidateTable({ 
  ...
  return (
    <section className="bg-surface-container-lowest border border-table-border rounded-lg overflow-hidden flex flex-col font-sans">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {candidate.source && (
                        <div className="text-[10px] text-on-surface-variant font-mono mt-1 opacity-90" title="Sumber Lamaran">
                    ...
                    <td className="px-4 py-3 text-right relative">
                      ...
                      {activeMenuId === candidate.id && (
                        <div className="absolute right-4 mt-1 w-44 bg-white border border-table-border rounded shadow-lg z-50 py-1 text-left animate-fadeIn">
                          ...
                          {(['Applied', 'Skill Test', 'Psychological Test', 'HR Interview', 'User Interview', 'Medical Check', 'Onboarding', 'Ditolak'] as CandidateStatus[]).map((st) => (
                            <button
                              key={st}
                              onClick={() => {
      ...
      {candidateToDelete && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-black/55 backdrop-blur-xs">
          <div className="bg-white border border-table-border rounded-xl max-w-md w-full shadow-2xl p-6 relative animate-scale-up">
            <div className="flex items-center gap-3 mb-4 text-status-error">
              ...
              <h3 className="text-base font-bold text-on-surface font-sans">Hapus Kandidat</h3>
            </div>
            <p className="text-xs text-on-surface-variant leading-relaxed mb-6 font-sans">
              Apakah Anda yakin ingin menghapus kandidat <strong className="text-on-surface font-semibold">{candidateToDelete.name}</strong> dari pipeline rekrutmen? Tindakan ini tidak dapat dibatalkan.
            </p>
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700 transition-colors shadow-sm cursor-pointer"
              >
                Hapus
              </button>
            </div>
