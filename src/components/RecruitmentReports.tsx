...
import PipelineFunnelDashboard from './PipelineFunnelDashboard';
...
export default function RecruitmentReports({ 
  ...
  const [reportTab, setReportTab] = useState<'executive' | 'pipeline' | 'spam'>('pipeline');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-table-border pb-1 space-x-6">
        <button
          onClick={() => setReportTab('pipeline')}
          className={`pb-2.5 px-1 text-xs font-bold tracking-wider uppercase border-b-2 transition-all flex items-center gap-1.5 cursor-pointer ${
            reportTab === 'pipeline' 
        ...
        >
          ...
          <span>Corong Pipeline & Proyek</span>
        </button>
        <button
        ...
        >
          ...
          <span>Detektor Spam Lamaran</span>
        </button>
      </div>

      {reportTab === 'pipeline' && (
        <PipelineFunnelDashboard candidates={candidates} />
      )}
