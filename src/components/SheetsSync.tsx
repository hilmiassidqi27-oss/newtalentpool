...
export default function SheetsSync({ candidates, onImportCandidates, onReplaceCandidates, logActivity }: SheetsSyncProps) {
  ...
  const userSpreadsheetId = '15lcJ5Z89Jz5snwkesaLOvH8jN3Vf5b8i6iCgvGDniQI';
  ...
  const [importPreview, setImportPreview] = useState<Partial<Candidate>[] | null>(null);
  ...
  // Load sheet workspace metadata on connect
  useEffect(() => {
    if (accessToken) {
  ...
  const handleConfirmImport = () => {
    ...
    try {
      ...
      logActivity('Import Competed', `Successfully imported ${importPreview.length} candidates from Google Sheets.`);
      alert(`Import Successful! Added ${importPreview.length} candidates to your local recruitment pipeline.`);
      setImportPreview(null);
      setImportSpreadsheetId('');
    } catch (err: any) {
      console.error(err);
      alert('Failed to import candidates: ' + err);
    } finally {
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {!googleUser ? (
            ...
            <div className="space-y-6">
              {/* SPECIAL FEATURE: Primary Linked Database Sheet (15lcJ5Z89Jz5snwkesaLOvH8jN3Vf5b8i6iCgvGDniQI) */}
              <div className="bg-gradient-to-br from-green-50/70 to-emerald-50/30 border border-green-200 rounded-lg p-6 space-y-5 shadow-sm relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-green-100">
                  ...
                  <a 
                    href="https://docs.google.com/spreadsheets/d/15lcJ5Z89Jz5snwkesaLOvH8jN3Vf5b8i6iCgvGDniQI/edit?usp=sharing"
                    target="_blank" 
