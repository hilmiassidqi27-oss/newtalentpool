...
export default function Sidebar({ activeTab, setActiveTab, onAddCandidate, onLogout, userRole, rolePermissions }: SidebarProps) {
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'geosource', label: 'GeoSource Dashboard', icon: Map },
    { id: 'jobs', label: 'Job Openings', icon: Briefcase },
    { id: 'pipeline', label: 'Pipeline Board', icon: Trello },
    { id: 'candidates', label: 'Candidates Archive', icon: Users },
  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 bg-surface-container-lowest border-r border-table-border flex flex-col py-6 z-50">
      ...
      {/* Navigation Links */}
      <nav className="flex-1 overflow-y-auto min-h-0 py-2 space-y-1">
      ...
      <div className="border-t border-table-border pt-4 space-y-1">
        {hasAccess('settings') && (
          ...
          >
            <Shield className="mr-3 w-5 h-5 shrink-0" />
            <span className="font-sans text-xs font-semibold">Kelola Akses</span>
          </button>
        )}
