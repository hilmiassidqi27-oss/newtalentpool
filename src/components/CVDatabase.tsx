...
export default function CVDatabase({ 
  ...
  const [cvToDelete, setCvToDelete] = useState<CVFile | null>(null);
  ...
  const confirmDeleteCv = () => {
    ...
    logActivity('CV Dihapus', `Menghapus dokumen CV ${cvToDelete.name} dari arsip.`);
    setCvToDelete(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Custom Confirmation Modal for Deleting CV */}
      {cvToDelete && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-black/55 backdrop-blur-xs">
          <div className="bg-white border border-table-border rounded-xl max-w-md w-full shadow-2xl p-6 relative animate-scale-up">
            <div className="flex items-center gap-3 mb-4 text-status-error">
              ...
              <h3 className="text-base font-bold text-on-surface font-sans">Hapus CV</h3>
            </div>
            <p className="text-xs text-on-surface-variant leading-relaxed mb-6 font-sans">
              Apakah Anda yakin ingin menghapus dokumen CV untuk <strong className="text-on-surface font-semibold">{cvToDelete.candidateName}</strong> ({cvToDelete.name}) dari basis data? Tindakan ini tidak dapat dibatalkan.
            </p>
