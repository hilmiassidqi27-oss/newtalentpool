...
export default function App() {
  ...
  const handleSaveCandidate = (savedData: Omit<Candidate, 'id'> & { id?: number }) => {
    if (savedData.id) {
      // Edit mode
      setCandidates(prev => 
        prev.map(c => c.id === savedData.id ? (savedData as Candidate) : c)
      );
      logActivity(
        'Updated Record', 
  ...
  const handleImportCandidates = (imported: Partial<Candidate>[]) => {
    setCandidates(prev => {
      ...
      return [...updatedOrNewCands, ...updatedList];
    });
  };
  ...
  const handleUpdateStatus = (id: number, status: CandidateStatus) => {
    setCandidates(prev => 
      prev.map(c => {
        if (c.id === id) {
          ...
          let hrResult = c.hrResult;
          let userResult = c.userResult;
          ...
          return { ...c, status, hrResult, userResult };
        }
        return c;
      })
    );
    ...
    if (updated) {
      logActivity(
        'Status Transferred', 
        `Moved ${updated.name} from ${updated.status} to ${status}`
      );
    }
  };
      ...
      <div className="ml-64 flex-1 flex flex-col min-h-screen">
        ...
        <main className="p-8 flex-1 space-y-8 max-w-[1440px] w-full mx-auto">
          ...
          {activeTab === 'talent' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-bold text-on-surface font-sans">Talent Pool & Archives</h3>
                <p className="text-xs text-on-surface-variant mt-1">Historical applicants profiles repository for future re-engagement.</p>
              </div>
              
              <div className="bg-white border border-table-border rounded-lg p-6 text-center py-12">
                <Users className="w-12 h-12 text-primary/30 mx-auto mb-4" />
                <h4 className="font-bold text-sm text-primary mb-1">Interactive Talent Bank Vault</h4>
                <p className="text-xs text-on-surface-variant max-w-md mx-auto leading-relaxed">
                  The talent bank keeps records of hundreds of past qualified riggers, welders and industrial operators with verified G-Suite files. 
                </p>
                <div className="mt-6 flex justify-center gap-2">
                  <button 
                    onClick={() => alert('Opening CV Vault Integration...')}
                    className="px-4 py-2 bg-primary text-white text-xs font-semibold rounded hover:bg-primary-container cursor-pointer"
                  >
                    Browse CV Database
                  </button>
                </div>
          ...
          {activeTab === 'reports' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-bold text-on-surface font-sans">Recruitment Analytics & Insights</h3>
                <p className="text-xs text-on-surface-variant mt-1">Statistical report generator for quarterly corporate pipeline progression.</p>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white border border-table-border rounded-lg p-6">
                  <h4 className="font-bold text-xs text-primary mb-4 uppercase tracking-wider">Candidate Split by Position</h4>
                    ...
                    {POSITIONS_LIST.map((pos) => {
                      ...
                      return (
                        <div key={pos} className="space-y-1">
                          <div className="flex justify-between text-xs">
                            <span className="text-on-surface-variant">{pos}</span>
                ...
                <div className="bg-white border border-table-border rounded-lg p-6 flex flex-col justify-between">
                  <div>
                    <h4 className="font-bold text-xs text-primary mb-4 uppercase tracking-wider">Pipeline Performance Audit</h4>
                    <div className="space-y-4 text-xs">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="text-status-success w-4.5 h-4.5 shrink-0" />
                        <span>Average screening duration: <strong>4.2 days</strong></span>
                      </div>
                      <div className="flex items-center gap-2">
                        <CheckCircle className="text-status-success w-4.5 h-4.5 shrink-0" />
                        <span>Candidate drop-off rate: <strong>14.5%</strong> (Healthy)</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <AlertCircle className="text-status-warning w-4.5 h-4.5 shrink-0" />
                        <span>High demand: <strong>Scaffolders</strong> recruitment is behind schedule</span>
                      </div>
                    </div>
                  </div>
                  <div className="pt-6 border-t border-table-border mt-6">
                    <button 
                      onClick={() => alert("Quarterly PDF Executive Pipeline Summary Generated!")}
                      className="w-full py-2 bg-primary text-white text-xs font-semibold rounded hover:bg-primary-container cursor-pointer flex items-center justify-center gap-2"
                    >
