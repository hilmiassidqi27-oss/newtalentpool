...
export async function importCandidatesFromSheet(
  ...
  return parseRawSheetData(values);
}

/**
 * Parses raw grid data (2D string array) from either Google Sheets API or a local uploaded Excel/CSV file.
 */
export function parseRawSheetData(values: string[][]): Partial<Candidate>[] {
